...
class HEBOTuner(BaseTuner):
    ...
    def __init__(
        ...
        """
        Initializes the HEBOTuner with the classification type, evaluation criterion, and tuning method.
    ...
    def holdout(
        ...
        Returns:
            Tuple[Dict[str, Union[float, int]], Optional[float]]: The best hyperparameters and the best threshold.
        """
        return self._run_optimization(
            learner, X_train_h, y_train_h, X_val, y_val, None, n_configs, n_jobs, verbosity
        )
    ...
    def cv(
        ...
        Args:
            learner (str): The machine learning model to evaluate.
            outer_splits (List[Tuple[pd.DataFrame, pd.DataFrame]]): List of cross-validation folds.
            n_configs (int): The number of configurations to evaluate during HPO.
            n_jobs (int): The number of parallel jobs for model training.
            verbosity (bool): Whether to print detailed logs during HEBO optimization.
    ...
    def _run_optimization(
        ...
        """
        General function to perform Bayesian Optimization using HEBO for both holdout and cross-validation.

        Args:
            learner (str): The machine learning model to evaluate.
            X_train_h (Optional[pd.DataFrame]): The training features for the holdout set (None if using CV).
            y_train_h (Optional[pd.Series]): The training labels for the holdout set (None if using CV).
            X_val (Optional[pd.DataFrame]): The validation features for the holdout set (None if using CV).
            y_val (Optional[pd.Series]): The validation labels for the holdout set (None if using CV).
            outer_splits (Optional[List[Tuple[pd.DataFrame, pd.DataFrame]]]): List of cross-validation folds (None if using holdout).
            n_configs (int): The number of configurations to evaluate during HPO.
            n_jobs (int): The number of parallel jobs for model training.
            verbosity (bool): Whether to print detailed logs during HEBO optimization.
        ...
        Returns:
            Tuple[Dict[str, Union[float, int]], Optional[float]]: The best hyperparameters and the best threshold.
        """
        model, search_space, params_func = Model.get(learner, self.classification, self.hpo)
        ...
        for i in range(n_configs):
            ...
            score = self._objective(
                model, params_dict, X_train_h, y_train_h, X_val, y_val, outer_splits, n_jobs
            )
        ...
        if self.classification == "binary" and self.criterion == "f1":
            if self.tuning == "holdout":
                ...
                best_threshold = self.threshold_optimizer.bo_threshold_optimization(probs, y_val)
            elif self.tuning == "cv":
    ...
    def _objective(
        ...
        Args:
            ...
            params_dict (Dict[str, Union[float, int]]): The suggested hyperparameters as a dictionary.
            X_train_h (Optional[pd.DataFrame]): The training features for the holdout set (None for CV).
            y_train_h (Optional[pd.Series]): The training labels for the holdout set (None for CV).
            X_val (Optional[pd.DataFrame]): The validation features for the holdout set (None for CV).
            y_val (Optional[pd.Series]): The validation labels for the holdout set (None for CV).
            outer_splits (Optional[List[Tuple[pd.DataFrame, pd.DataFrame]]]): List of cross-validation folds (None for holdout).
            n_jobs (int): The number of parallel jobs for model training.
        ...
        score = self.trainer.evaluate_objective(
            self.tuning, model_clone, X_train_h, y_train_h, X_val, y_val, outer_splits, n_jobs
        )
